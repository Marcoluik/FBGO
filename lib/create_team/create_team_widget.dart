...
class CreateTeamWidget extends StatefulWidget {
  const CreateTeamWidget({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<TeamsRecord>>(
      ...
      builder: (context, snapshot) {
        ...
        List<TeamsRecord> createTeamTeamsRecordList = snapshot.data!;
        // Return an empty Container when the item does not exist.
        if (snapshot.data!.isEmpty) {
          return Container();
        }
        final createTeamTeamsRecord = createTeamTeamsRecordList.isNotEmpty
            ? createTeamTeamsRecordList.first
            : null;
        return Scaffold(
          key: scaffoldKey,
                    ...
                    child: FFButtonWidget(
                      onPressed: () async {
                        final teamsCreateData = createTeamsRecordData(
                          tname: _model.yourNameController.text,
                          tbio: _model.myBioController.text,
                          tpfp: _model.uploadedFileUrl,
                        );
                        await TeamsRecord.collection.doc().set(teamsCreateData);

                        final usersUpdateData = createUsersRecordData(
                          team: createTeamTeamsRecord!.reference.id,
                        );
